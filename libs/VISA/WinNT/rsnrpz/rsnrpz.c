...
/*                                                                           */
/*  Rev 3.6.0.1  31-JUL-2019 Juergen D. Geltinger                            */
/*  - Extended max. number of sensors from 32 to 63                          */
/*                                                                           */
...
/*                                                                           */
/*  Rev 3.2.2.1  18-AUG-2016 Juergen D. Geltinger                            */
/*  - Added WG type to 'well-know' sensor list                               */
/*                                                                           */
...
static const char* const RSNRPZ_REVISION        = "Rev %d.%d.%d.%d, %02d/%04d"; /* Instrument driver revision                 */

#define BUFFER_SIZE                             2048L                           /* File I/O buffer size                       */
#define RSNRPZ_MAX_CHANNELS                     64                              /* Maximum number of channels                 */
#define RSNRPZ_MAX_MAIN_SESSIONS                64                              /* Maximum number of main sessions            */
...
static const char* const RSNRPZ_IDN_NRP         = "ROHDE&SCHWARZ,NRP";
...
//===============================================
#define NUM_MEMTRACE      200
#define LEN_MEMTRACE_LINE 256
typedef char   t_MemTraceLn[LEN_MEMTRACE_LINE];
typedef char * t_MemTrace[NUM_MEMTRACE];
static t_MemTrace *gs_pMemTrace  = 0;
static int         iMemTraceLine = 0;

void CloseMemTrace( void )
{
  int i = 0;
...
void OpenMemTrace( void )
{
  if ( gs_pMemTrace )
    CloseMemTrace();

  gs_pMemTrace = (t_MemTrace*)calloc( NUM_MEMTRACE+1, sizeof( char * ) );
  if ( gs_pMemTrace )
  {
    int i = 0;
    for ( i = 0; i < NUM_MEMTRACE; i++ )
      (*gs_pMemTrace)[i] = (char*)calloc( LEN_MEMTRACE_LINE, sizeof( char ) );
  }
  iMemTraceLine = 0;
}

void TraceMem( const char* cpFmt, ... )
{
  va_list ap;
  char    szTime[40];
  char    szLine[200];
...
#if defined(__APPLE__)
  // from mach/mach_time.h
  sprintf( szTime, "%ld\t: ", (long int)mach_absolute_time() );
#else
  #if defined(__linux__)
    // from sys/sysinfo.h
    struct sysinfo si;
    if( sysinfo(&si) == 0 )
      sprintf( szTime, "%ld\t: ", (long int)si.uptime );
  #else
    sprintf( szTime, "%ld\t: ", GetTickCount() );
  #endif
#endif
  ...
  vsnprintf(
#endif
    szLine, sizeof( szLine ), cpFmt, ap );
  va_end( ap );

  if ( iMemTraceLine < NUM_MEMTRACE )
  {
    strcpy( (*gs_pMemTrace)[ iMemTraceLine ], szTime );
    strcat( (*gs_pMemTrace)[ iMemTraceLine ], szLine );

    iMemTraceLine++;
  }
}

char **GetMemTrace( void )
{
  return *gs_pMemTrace;
}
//===============================================

// Support for WinCE7 (Unicode build)
#ifdef _UNICODE
const WCHAR *CH2WCH( const char *cp )
{
  static WCHAR wszBuf[1024];
  mbstowcs( wszBuf, cp, 1023 );
  return wszBuf;
}
const char *WCH2CH( const WCHAR *cp )
{
  static char szBuf[1024];
  wcstombs( szBuf, cp, 1023 );
  return szBuf;
}
#endif
...
struct tag_SSessionInfo
{
    ...
    ViChar*                 ArrayBinaryData;
    ViInt32                 ArrayBinaryDataSize;
...
static int  IsWellKnownSensor( const ViChar *cpIDN );
...
static SRangeTableEntry gs_TriggerSourceRangeTable[] =
{
    ...
    {"EXT2",(1L << 5), RSNRPZ_TRIGGER_SOURCE_EXT2       },
    {LAST_RANGE_TABLE_ENTRY                             }
};
 ...
 * TRIGger:MASTer:PORT
 */
static SRangeTableEntry gs_TriggerMasterPortTable[] =
{
    {"EXT", (1L << 4), RSNRPZ_PORT_EXT  },
    {"EXT2",(1L << 5), RSNRPZ_PORT_EXT2 },
    {LAST_RANGE_TABLE_ENTRY             }
};
 ...
 * TRIGger:SYNC:PORT
 */
static SRangeTableEntry gs_TriggerSyncPortTable[] =
{
    {"EXT", (1L << 4), RSNRPZ_PORT_EXT  },
    {"EXT2",(1L << 5), RSNRPZ_PORT_EXT2 },
    {LAST_RANGE_TABLE_ENTRY             }
 ...
 * LED Mode commands
 */
static SRangeTableEntry gs_LedModeTable[] =
{
    {"USER",  (1L << 0), RSNRPZ_LEDMODE_USER   },
    {"SENS",  (1L << 1), RSNRPZ_LEDMODE_SENSOR },
    { LAST_RANGE_TABLE_ENTRY                   }
};
...
{
    CMD("CALibration:DATA",                 1, 1,  NULL)   /* Factory Calibration Data Binary Block */
    CMD("CALibration:DATA:LENGth",          1, 3,  NULL)   /* Long */
    CMD("CALibration:USER:DATA",           20, 1,  NULL)   /* User Calibration Data Binary Block */
    CMD("CALibration:USER:DATA:LENGth",    20, 2,  NULL)   /* Long */
    CMD("SYSTem:REBoot",                   20, 4,  NULL)
    CMD("SYSTem:LED:COLor",                20, 25, NULL)
    CMD("SYSTem:LED:MODE",                 20, 26, gs_LedModeTable)           /* Bitfield */
    CMD("SYSTem:NAME",                     20, 27, NULL)
    CMD("TRIGger:MASTer:PORT",             20, 28, gs_TriggerMasterPortTable) /* Bitfield */
    CMD("TRIGger:SYNC:PORT",               20, 29, gs_TriggerSyncPortTable)   /* Bitfield */
...
#define EVT_CHGRP_SINGLE  -1
#define EVT_CHGRP_BUFFER  -2
#define EVT_CHGRP_CALIB   -3
...
/* Purpose:  Sets all member to initial state (0)                            */
/*===========================================================================*/
static void ClearMainSession( struct tag_SMainSession *pMainSession )
{
  if ( ! pMainSession )
    return;

#if ! defined(__linux__) && ! defined(__APPLE__)
  memset( pMainSession, 0x00, sizeof( *pMainSession ) );
#else
  cs;             // is a class under Linux & Apple
  memset( &pMainSession->cs, 0x00, sizeof( pMainSession->cs ) );

  pMainSession->event         = 0;
  pMainSession->MessageID     = 0;
  pMainSession->ErrorChecking = 0;
  memset( pMainSession->pSessions,    0x00, sizeof( pMainSession->pSessions )    );
  memset( pMainSession->pSInfos,      0x00, sizeof( pMainSession->pSInfos )      );
  memset( pMainSession->NrpRegisters, 0x00, sizeof( pMainSession->NrpRegisters ) );
  memset( pMainSession->EventTable,   0x00, sizeof( pMainSession->EventTable )   );
  memset( pMainSession->NrpErrors,    0x00, sizeof( pMainSession->NrpErrors )    );
  memset( pMainSession->hHandlesRd,   0x00, sizeof( pMainSession->hHandlesRd )   );
  memset( pMainSession->hHandlesWr,   0x00, sizeof( pMainSession->hHandlesWr )   );
#endif
}
...
/* Purpose:  Sets all member to initial state (0)                            */
/*===========================================================================*/
static void ClearSessionInfo( struct tag_SSessionInfo *pSessionInfo )
{
  if ( ! pSessionInfo )
    return;

#if ! defined(__linux__) && ! defined(__APPLE__)
  memset( pSessionInfo, 0x00, sizeof( *pSessionInfo ) );
#else
  cs;             // is a class under Linux & Apple
  memset( &pSessionInfo->cs, 0x00, sizeof( pSessionInfo->cs ) );

  memset( pSessionInfo->SensorType, 0x00, sizeof( pSessionInfo->SensorType ) );
  pSessionInfo->MainSession            = 0;
  pSessionInfo->CacheTable             = 0;
  pSessionInfo->IsSingleMeasValid      = 0;
  pSessionInfo->IsArrayMeasValid       = 0;
  pSessionInfo->IsArrayMeasAuxValid    = 0;
  pSessionInfo->IsMeasurementComplete  = 0;
  pSessionInfo->SingleMeas             = 0;
  pSessionInfo->SingleMeasAux1         = 0;
  pSessionInfo->SingleMeasAux2         = 0;
  pSessionInfo->ArrayMeas              = 0;
  pSessionInfo->ArrayMeasAux1          = 0;
  pSessionInfo->ArrayMeasAux2          = 0;
  pSessionInfo->ArrayMeasSize          = 0;
  pSessionInfo->ArrayBinaryData        = 0;
  pSessionInfo->ArrayBinaryDataSize    = 0;
  pSessionInfo->IsInStatMode           = 0;
  pSessionInfo->IsPulseMeasurementOn   = 0;
  pSessionInfo->IsPulseScalarsComplete = 0;
  pSessionInfo->PulseHighestParamNo    = 0;
#if defined (USE_AAV_PATCH)
  pSessionInfo->IsAutoAvgEnabled       = 0;
#endif
#endif
}
...
// The function returns zero if no valid pointer was found
static SMainSession* V2N_Lock( ViSession lSess )
{
  SMainSession *pSessionRet = 0;
  ...
  pSessionRet = V2NTable[ lSess-1 ];

  if ( pSessionRet )
    EnterCriticalSection( &pSessionRet->cs );
...
static ViSession N2V( SMainSession *pMainSession )
{
    int iIdx;
...
static int IsWellKnownSensor( const ViChar *cpIDN )
{
  if ( strstr(cpIDN, RSNRPZ_IDN_QUERY) )
    return 1;

  if ( strstr(cpIDN, RSNRPZ_IDN_QUERY_311) )
    return 1;

  if ( strstr(cpIDN, RSFSH_IDN_QUERY) )
    return 1;

  if ( strstr(cpIDN, RSNRPC_IDN_QUERY) )
    return 1;
  ...
  // '....,NRP18SN' or
  // '....,NRP33SN' etc.
  if ( strstr( cpIDN, RSNRPZ_IDN_NRP ) )
  {
    if ( strstr( cpIDN + strlen( RSNRPZ_IDN_NRP ), "S," ) )
      return 1;

    if ( strstr( cpIDN + strlen( RSNRPZ_IDN_NRP ), "SN," ) )
      return 1;
    ...
    if ( strstr( cpIDN + strlen( RSNRPZ_IDN_NRP ), "T," ) )
      return 1;

    if ( strstr( cpIDN + strlen( RSNRPZ_IDN_NRP ), "TN," ) )
      return 1;

    if ( strstr( cpIDN + strlen( RSNRPZ_IDN_NRP ), "W," ) )
      return 1;

    if ( strstr( cpIDN + strlen( RSNRPZ_IDN_NRP ), "WN," ) )
      return 1;

    if ( strstr( cpIDN + strlen( RSNRPZ_IDN_NRP ), "A," ) )
      return 1;

    if ( strstr( cpIDN + strlen( RSNRPZ_IDN_NRP ), "AN," ) )
      return 1;

    if ( strstr( cpIDN + strlen( RSNRPZ_IDN_NRP ), "WG," ) )
      return 1;
  }
...
)
{
    ...
    TraceMem( "(Rsc=%s, ID=%d, Reset=%d, LongDist=%d )", resourceName, IDQuery, resetDevice, bLongDistance );
    ...
    {
      rsnrpz_status = NrpOpenDriver();
      TraceMem( "NrpOpenDriver() returns 0x%08x", rsnrpz_status );
    ...
    if ( bLongDistance == VI_TRUE )
        rsnrpz_status = NrpSetAttribute( 0, NRP_ATTR_DISTANCE_LONG, 0 );
    else
        rsnrpz_status = NrpSetAttribute( 0, NRP_ATTR_DISTANCE_DEFAULT, 0 );

    TraceMem( "NrpSetAttribute() returns 0x%08x", rsnrpz_status );


    rsnrpz_status = NrpOpenSensor( resourceName, &session);
    //
    // The NrpOpenSensor() call may have returned an
    // NRP_ERROR_SENSOR_IN_USE code. That means, the
    // sensor with the given name ('resourceName') is
    // already opened by another application.
    //
    TraceMem( "NrpOpenSensor( %s ) returns 0x%08x, session = 0x%08x", resourceName, rsnrpz_status, session );
    
    if ( rsnrpz_status == NRP_ERROR_SENSOR_IN_USE )
    {
      if ( bLocked == VI_TRUE )
    ...
    // ------------------- LABVIEW PATCH -------------------
    //
    // In LabView breaking a running program leaves the
    // NRP_SESSION open under the current process id (PID).
    // The next rsnrpz_init() would erroneously add another
    // 'SSessionInfo' if we do not check. Therefore we
    // search the list of sessions for a possibly already
    // existing entry for the currently opened sensor...
    // -----------------------------------------------------
    psession_info = GetSesInfoFromSession( session );
    if ( psession_info )
    {
      pmain_session = psession_info->MainSession;
      *pInstrSession = N2V( pmain_session );
      ...
      TraceMem( "OK: opened an already open sensor" );
      return VI_SUCCESS;
    }
    // ------------------- LABVIEW PATCH -------------------
    ...
    ClearSessionInfo( psession_info );
    ...
    TraceMem( "InitializeCriticalSection( pmain_session_info->cs ) done" );
    nb_of_commands = rsnrpz_GetNbOfCommands();
    TraceMem( "rsnrpz_GetNbOfCommands() returns %d", nb_of_commands );
    ...
    *pInstrSession = 0;
    ...
    TraceMem( "AddSessionNode() done" );
    *pInstrSession = N2V( pmain_session );
    TraceMem( "N2V() InstrSession returns %d", *pInstrSession );
     ...
     *  - *RST         [11/ 1]
     */
    rsnrpz_status = rsnrpz_GetIndexFromPacket(8, 2, &index);
    TraceMem( "rsnrpz_GetIndexFromPacket(8, 2) returns 0x%08x, index = %d", rsnrpz_status, index );
    if ( rsnrpz_status < VI_SUCCESS )
      goto Error;
    psession_info->CacheTable[index].IsAvailable = 1;

    rsnrpz_status = rsnrpz_GetIndexFromPacket(8, 10, &index);
    TraceMem( "rsnrpz_GetIndexFromPacket(8, 10) returns 0x%08x, index = %d", rsnrpz_status, index );
    if ( rsnrpz_status < VI_SUCCESS )
      goto Error;
    psession_info->CacheTable[index].IsAvailable = 1;

    rsnrpz_status = rsnrpz_GetIndexFromPacket(8, 11, &index);
    TraceMem( "rsnrpz_GetIndexFromPacket(8, 11) returns 0x%08x, index = %d", rsnrpz_status, index );
    if ( rsnrpz_status < VI_SUCCESS )
      goto Error;
    psession_info->CacheTable[index].IsAvailable = 1;

    rsnrpz_status = rsnrpz_GetIndexFromPacket(11, 3, &index);
    TraceMem( "rsnrpz_GetIndexFromPacket(11, 3) returns 0x%08x, index = %d", rsnrpz_status, index );
    if ( rsnrpz_status < VI_SUCCESS )
      goto Error;
    psession_info->CacheTable[index].IsAvailable = 1;

    rsnrpz_status = rsnrpz_GetIndexFromPacket(11, 1, &index);
    TraceMem( "rsnrpz_GetIndexFromPacket(11, 1) returns 0x%08x, index = %d", rsnrpz_status, index );
    if ( rsnrpz_status < VI_SUCCESS )
      goto Error;
    ...
    rsnrpz_status = rsnrpz_InitializeRegisters( pmain_session->NrpRegisters);
    TraceMem( "rsnrpz_InitializeRegisters() returns 0x%08x", rsnrpz_status );
    if ( rsnrpz_status < VI_SUCCESS )
      goto Error;
  ...
  #pragma warning (disable: 4028 )
    ...
    rsnrpz_status = NrpSetNotifyCallbackDataAvailable( NrpDataAvailableCallback,  0);
    TraceMem( "NrpSetNotifyCallbackDataAvailable(      NrpDataAvailableCallback,  0) returns 0x%08x", rsnrpz_status );
    if ( rsnrpz_status < VI_SUCCESS )
      goto Error;

    rsnrpz_status = NrpSetNotifyCallbackErrorOccurred( NrpErrorOccurredCallback,  0);
    TraceMem( "NrpSetNotifyCallbackErrorOccurred(      NrpErrorOccurredCallback,  0) returns 0x%08x", rsnrpz_status );
    if ( rsnrpz_status < VI_SUCCESS )
      goto Error;

    rsnrpz_status = NrpSetNotifyCallbackStateChanged( NrpStateChangedCallback,  0);
    TraceMem( "NrpSetNotifyCallbackStateChanged(      NrpStateChangedCallback,  0) returns 0x%08x", rsnrpz_status );
    if ( rsnrpz_status < VI_SUCCESS )
      goto Error;

    rsnrpz_status = NrpSetNotifyCallbackCommandAccepted( NrpCommandAcceptedCallback,  0);
    TraceMem( "NrpSetNotifyCallbackCommandAccepted(      NrpCommandAcceptedCallback,  0) returns 0x%08x", rsnrpz_status );
    if ( rsnrpz_status < VI_SUCCESS )
      goto Error;
  ...
  #pragma warning (pop)
    ...
    /*- Identification Query ------------------------------------------------*/
    {
        ...
        TraceMem( "Sending *IDN..." );
        rsnrpz_status = rsnrpz_ReadValue( *pInstrSession, 1, (char*)"*IDN", buffer, sizeof(buffer) );
        TraceMem( "rsnrpz_ReadValue( *IDN ) returns 0x%08x, buffer = '%s'", rsnrpz_status, buffer );
        if ( rsnrpz_status < VI_SUCCESS )
          goto Error;
        ...
        if( IDQuery )
        {
            if ( ! IsWellKnownSensor( buffer ) )
            {
              TraceMem( "VI_WARN_UNKNOWN_SENSOR" );
              CHECKERR_CU( VI_WARN_UNKNOWN_SENSOR );
            }
        }
        sscanf (buffer,"%*[^,],%[^,]",psession_info->SensorType);
        TraceMem( "sscanf() returns SensorType = %s", psession_info->SensorType );
    }

    /*- Reset instrument ----------------------------------------------------*/
    if ( resetDevice )
    {
        rsnrpz_status = rsnrpz_chan_reset( *pInstrSession, 1 );
        TraceMem( "rsnrpz_chan_reset() returns 0x%08x", rsnrpz_status );
        if ( rsnrpz_status < VI_SUCCESS )
          goto Error;
    }
    else  /*- Send Default Instrument Setup ---------------------------------*/
    {
        rsnrpz_status = rsnrpz_defaultInstrSetup( *pInstrSession, 1 );
        TraceMem( "rsnrpz_defaultInstrSetup() returns 0x%08x", rsnrpz_status );
        if ( rsnrpz_status < VI_SUCCESS )
          goto Error;
    }

    rsnrpz_status = rsnrpz_checkStatus( *pInstrSession );
    TraceMem( "rsnrpz_checkStatus() returns 0x%08x", rsnrpz_status );
    if ( rsnrpz_status < VI_SUCCESS )
      goto Error;

    rsnrpz_status = rsnrpz_SetBitInRegisters( pmain_session, DEVICE_STATUS_REGISTER_CONNECTED, CONDITION_REGISTER, 1, VI_TRUE );
    TraceMem( "rsnrpz_SetBitInRegisters() returns 0x%08x", rsnrpz_status );
    if ( rsnrpz_status < VI_SUCCESS )
      goto Error;

Error:
    if ( rsnrpz_status < VI_SUCCESS )
      TraceMem( "Error handling" );
    ...
    if ( rsnrpz_status < VI_SUCCESS )
    {
      NRP_SESSION nrpSess = 0;
      if ( pmain_session != 0 )
        if ( pmain_session->pSessions )
          nrpSess = pmain_session->pSessions[0];

      TraceMem( "Resetting events..." );
      if ( (pmain_session != 0 ) && pmain_session->event )
        ResetEvent( pmain_session->event );

      TraceMem( "Calling NrpSetNotifyCallbackDataAvailable(0,0)" );
      NrpSetNotifyCallbackDataAvailable(   NULL, 0);
      TraceMem( "Calling NrpSetNotifyCallbackErrorOccurred(0,0)" );
      NrpSetNotifyCallbackErrorOccurred(   NULL, 0);
      TraceMem( "Calling NrpSetNotifyCallbackStateChanged(0,0)" );
      NrpSetNotifyCallbackStateChanged(    NULL, 0);
      TraceMem( "Calling NrpSetNotifyCallbackCommandAccepted(0,0)" );
      NrpSetNotifyCallbackCommandAccepted( NULL, 0);

      if ( nrpSess )
      {
        NrpEmptyAllBuffers( nrpSess );
        NrpCloseSensor( nrpSess );
        TraceMem( "NrpCloseSensor() done" );
        nrpSess = 0;
      }

      if ( pInstrSession )
        *pInstrSession = (ViSession)nrpSess;  // nrpSess IS 0, therefore
                                              // this assignment is okay

      TraceMem( "Clearing session information..." );
      if ( psession_info != 0 )
      {
      ...
      TraceMem( "Clearing main session..." );
      if ( pmain_session != 0 )
      {
        memset( pmain_session->pSessions, 0, sizeof(NRP_SESSION) * RSNRPZ_MAX_CHANNELS );

        CloseHandle( pmain_session->event );
        TraceMem( "CloseHandle() [=Event] done" );
        free( pmain_session );
      }
      ...
      TraceMem( "ERR: returning status 0x%08x", rsnrpz_status );
      return rsnrpz_status; // possibly NRP_ERROR_SENSOR_IN_USE
    ...
    TraceMem( "OK: returning status 0x%08x", rsnrpz_status );
    return rsnrpz_status;
}
...
#if ! defined(__linux__) && ! defined(__APPLE__) && ! defined(UNDER_CE)
...
)
{
    ...
    psession_info = (SSessionInfo*)malloc(sizeof(SSessionInfo));
    ClearSessionInfo( psession_info );
    psession_info->MainSession = pmain_session;
...
Error:
    if ( rsnrpz_status < (ViStatus) NRP_SUCCESS )
    {
      ...
      if ( is_open )
      {
        NrpEmptyAllBuffers( pmain_session->pSessions[channel-1] );
...
)
{
    ...
    if ( pmain_session )
    {
        ...
        // Need only be unlocked when pmain_session ain't NULL
        LeaveCriticalSection( &pmain_session->cs );
    }
...
)
{
    ...
    for (i = 1; i <= RSNRPZ_MAX_CHANNELS && is_meas_complete; i++)
    {
      ...
      if ( pmain_session )
      {
        nrpSess = pmain_session->pSessions[i-1];
        psession_info = pmain_session->pSInfos[i-1];
        LeaveCriticalSection( &pmain_session->cs );
      }

      if ( nrpSess && psession_info )
      {
    ...
    if ( is_meas_complete )
    {
       ...
       */
      NRP_SESSION nrpSess0 = 0;
      SSessionInfo* psession_info0 = 0;
      ...
      if ( pmain_session )
      {
        nrpSess0 = pmain_session->pSessions[0];
        psession_info0 = pmain_session->pSInfos[0];
        LeaveCriticalSection( &pmain_session->cs );
      }

      if ( nrpSess0 && psession_info0 )
      {
          EnterCriticalSection(&psession_info0->cs);
          psession_info0->IsMeasurementComplete  = VI_FALSE;
          psession_info0->IsPulseScalarsComplete = VI_FALSE;
          LeaveCriticalSection(&psession_info0->cs);
      }
    }
...
/* Purpose:  This function returns header and value of a certain line within */
/*           the result of the SYST:INFO? query                              */
/*           SYST:INFO? query returns an array of strings, where every line  */
/*           denotes a certain sensor parameter. -- For example, SYST:INFO?  */
/*           could return                                                    */
...
/*                  ::                                                       */
/*                                                                           */
/*           In this example a call of rsnrpz_chan_infoHeader() with a       */
/*           parameterNb = 6 would return the 6th line, which contains the   */
/*           sensor Firmware version (--> "SW Build:01.37" in this case)     */
/*===========================================================================*/
#undef  __FUNC__
...
/* Purpose:  This function returns the number of lines (strings) which are   */
/*           returned by a SYST:INFO? query on the selected channel.         */
/*                                                                           */
...
#if defined (USE_AAV_PATCH)
    //--------------------------------------------------
    // Patch to cure NRPxxY[N] AutoAvg behavior
    //--------------------------------------------------
    if ( channel > 0 && channel <= RSNRPZ_MAX_CHANNELS )
    {
      psession_info = pmain_session->pSInfos[channel-1];

      if ( psession_info->IsAutoAvgEnabled == VI_TRUE )
        rsnrpz_chans_abort( vi );
    }
    //--------------------------------------------------
#endif
...
)
{
    ...
    static rsnrpz_tStringValPair statusDescArray[] = {
        ...
        {VI_WARN_UNKNOWN_SENSOR,                 "WARNING: Sensor Type unknown"},
...
/* Purpose:  This function returns the DRIVER revision (only).               */
/*           Although a parameter for the INSTRUMENT revision exists,        */
/*           the instrument revision is NOT set by this function, but an     */
/*           empty string is always returned for the instrument revision.    */
/*===========================================================================*/
#undef  __FUNC__
...
//  CHECKSESSION(vi);
    ...
    if ( driverRev )
    {
        sprintf( driverRev, RSNRPZ_REVISION, 
                            VER_PRODUCTMAJORVERSION,
                            VER_PRODUCTMINORVERSION,
                            VER_BUILDMAJORVERSION,
                            VER_BUILDMINORVERSION,
                            VER_BUILDMONTH,
                            VER_BUILDYEAR
               );
    }
...
)
{
    ...
    { /* inner block */
      ...
      nrpSess       = pmain_session->pSessions[channel-1];
      psession_info = pmain_session->pSInfos[channel-1];
...
/*===========================================================================*/
/* Function: (internally) close all sensors from a session                   */
/*           (frequently only 1 sensor on a session)                         */
/*===========================================================================*/
#undef  __FUNC__
#define __FUNC__ "rsnrpz_close_session"
static ViStatus _VI_FUNC rsnrpz_close_session( ViSession vi, ViBoolean bDoReset )
{
  ViStatus        rsnrpz_status = VI_SUCCESS;
  ViStatus        reset_status  = VI_SUCCESS;
  SMainSession*   pmain_session = NULL;
  SSessionInfo*   psession_infos_closed[RSNRPZ_MAX_CHANNELS];
  ViInt32         i             = 0;
  ViInt32         j             = 0;
  ViInt32         iCloseCnt     = 0;
  ViInt32         command_count = rsnrpz_GetNbOfCommands();
  ...
  pmain_session = V2N_Lock( vi );  // Might be 0

  if ( pmain_session == 0 )
  {
    if ( vi != 0 )                // This can happen if somebody tries
      return VI_ERROR_PARAMETER1; // to close a session twice...
  ...
  if ( bDoReset )
  {
    // Must unlock the session because _WriteCommand() locks it again
    LeaveCriticalSection( &pmain_session->cs );
  ...
  memset( psession_infos_closed, 0, sizeof( psession_infos_closed ) );
  ...
  for ( i = 0; i < RSNRPZ_MAX_CHANNELS; i++ )
  {
    ...
    if ( pmain_session->pSessions[i] != 0 )
    {
      ...
      nrpSess = pmain_session->pSessions[i];
      NrpEmptyAllBuffers( nrpSess );
      NrpCloseSensor( nrpSess );
      nrpSess = 0;
      pmain_session->pSessions[i] = 0;

      psession_infos_closed[ iCloseCnt ] = psession_info;
      iCloseCnt++;
    }
  }
   ...
   *=====================================================================*/
  for ( i = 0; psession_infos_closed[i]; i++ )
  {
    DeleteCriticalSection(&(psession_infos_closed[i]->cs));

    if ( psession_infos_closed[i]->CacheTable != NULL )
    {
      for ( j = 0; j < command_count; j++)
      {
        if (( psession_infos_closed[i]->CacheTable[j].type == DATA_STRING ) &&
            ( psession_infos_closed[i]->CacheTable[j].current.pstring != NULL ))
        {
          free(psession_infos_closed[i]->CacheTable[j].current.pstring);
          psession_infos_closed[i]->CacheTable[j].current.pstring = NULL;
        }
      }

      free( psession_infos_closed[i]->CacheTable);
      psession_infos_closed[i]->CacheTable = NULL;

      if ( psession_infos_closed[i]->ArrayMeas )
      {
        free(psession_infos_closed[i]->ArrayMeas);
        psession_infos_closed[i]->ArrayMeas = NULL;
        psession_infos_closed[i]->ArrayMeasSize = 0;
      }

      if ( psession_infos_closed[i]->ArrayBinaryData )
      {
        free( psession_infos_closed[i]->ArrayBinaryData );
        psession_infos_closed[i]->ArrayBinaryData     = NULL;
        psession_infos_closed[i]->ArrayBinaryDataSize = 0;
      }

      free( psession_infos_closed[i] );
      psession_infos_closed[i] = NULL;
    }
  }
...
{
  return rsnrpz_close_session( vi, VI_FALSE );
}
...
/*           that all IsAvailable flags are set to zero. This is done in init*/
/*           when driver calls memset for cache zeroing                      */
/*===========================================================================*/
#undef  __FUNC__
...
#if defined (USE_AAV_PATCH)
    ViInt16         auto_avg_param = -1;
#endif

    // get associated session and check
    SMainSession *pmain_session = V2N_Lock( vi );
    if( pmain_session==0 )
        return VI_ERROR_PARAMETER1;
...
#if defined (USE_AAV_PATCH)
    /*
     * SENSe:AVERage:COUN:AUTO
     */
    if ((gs_NrpCommands[index].groupNb == 3) &&
        (gs_NrpCommands[index].paramNb == 2))
    {
        auto_avg_param = (ViInt16)( *((ViInt32*)parameter) );
        trace_log( "SENSe:AVERage:COUN:AUTO : %d ", (int)auto_avg_param );
    }
#endif
...
#if defined (USE_AAV_PATCH)
                if ( (auto_avg_param == VI_FALSE) || (auto_avg_param == VI_TRUE) )
                  psession_info->IsAutoAvgEnabled = auto_avg_param;
#endif
...
{
    ViStatus        rsnrpz_status = VI_SUCCESS;
    ViStatus        iTrace        = VI_SUCCESS;
    ViChar          buffer[BUFFER_SIZE] = "";

    TraceMem( "Now in rsnrpz_defaultInstrSetup()" );

//  CHECKERR( rsnrpz_ReadValue( vi, channel, (char*)"*IDN", buffer, sizeof(buffer) ) );
    rsnrpz_status = rsnrpz_ReadValue( vi, channel, (char*)"*IDN", buffer, sizeof(buffer) );
    TraceMem( "rsnrpz_ReadValue( *IDN ) returns 0x%08x", rsnrpz_status );
    if ( rsnrpz_status < VI_SUCCESS )
      return rsnrpz_status;

    iTrace = rsnrpz_WriteCommand( vi, channel, (char*)"SYST:FEAT?", NULL );
    TraceMem( "rsnrpz_WriteCommand( SYST:FEAT? ) returns 0x%08x", iTrace );

    if (iTrace != VI_SUCCESS)
    {
        TraceMem( "Try to clear instrument error by rsnrpz_checkStatus()" );

        // remove instrument errors
        iTrace = rsnrpz_checkStatus( vi );
        TraceMem( "rsnrpz_checkStatus() returns 0x%08x", iTrace );

        // fall back to request the features
//      CHECKERR( rsnrpz_WriteCommand( vi, channel, (char*)"SYST:INIT", NULL));
        rsnrpz_status = rsnrpz_WriteCommand( vi, channel, (char*)"SYST:INIT", NULL );
        TraceMem( "rsnrpz_WriteCommand( SYST:INIT ) returns 0x%08x", rsnrpz_status );
        if ( rsnrpz_status < VI_SUCCESS )
          return rsnrpz_status;
    }
    else
...
//      CHECKERR( rsnrpz_WriteCommand( vi, channel, (char*)"SYST:LIM?", NULL));
        rsnrpz_status = rsnrpz_WriteCommand( vi, channel, (char*)"SYST:LIM?", NULL);
        TraceMem( "rsnrpz_WriteCommand( SYST:LIM? ) returns 0x%08x", rsnrpz_status );
        if ( rsnrpz_status < VI_SUCCESS )
          return rsnrpz_status;
    }

    TraceMem( "Now leaving rsnrpz_defaultInstrSetup() returning 0x%08x", rsnrpz_status );
    return rsnrpz_status;
}
...
#ifdef _UNICODE
    pmain_session->EventTable[i].event   = CreateEvent( 0, TRUE, FALSE, CH2WCH(event_name) );
#else
...
)
{
     ...
     * pmain_session = V2N_Lock( vi );
     *
     * because the caller of RemoveFirstError()
     * has already locked the main session!
     */

    if ((vi < 1) || (vi > RSNRPZ_MAX_MAIN_SESSIONS))
      return VI_ERROR_PARAMETER1;

    pmain_session = V2NTable[vi - 1];
    if (pmain_session == 0)
      return VI_ERROR_PARAMETER1;
...
trace_log( "DATA_BINARYBLOCK" );
            ...
            if ( psession_info->ArrayBinaryData )
            {
              free( psession_info->ArrayBinaryData );
              psession_info->ArrayBinaryData     = NULL;
              psession_info->ArrayBinaryDataSize = 0;
            }

            psession_info->ArrayBinaryData     = (ViChar *)malloc( size );
            psession_info->ArrayBinaryDataSize = size;

            CHECKERR_CU( NrpGetBinaryResult( session,
                                            (unsigned char*)psession_info->ArrayBinaryData,
                                             psession_info->ArrayBinaryDataSize,
                                            &read_count) );

            if ( psession_info->ArrayBinaryDataSize != read_count )
            {
              // Tell the user some error...
            }
            rsnrpz_SetEvent(session, EVT_CHGRP_CALIB, EVT_CHGRP_CALIB );
        }
        break;
...
trace_log( "DATA_FLOATARRAY" );
            ...
            rsnrpz_SetEvent( session, EVT_CHGRP_BUFFER, EVT_CHGRP_BUFFER );
        }
        break;
...
trace_log( "DATA_FLOATRESULT" );
            ...
            rsnrpz_SetEvent(session, EVT_CHGRP_SINGLE, EVT_CHGRP_SINGLE );
        }
        break;
...
/*                                                                           */
/*           HINT: Due to syntax definitions the first parameter must be of  */
/*                 type 'ViSession', but it is not used and should be chosen */
/*                 as 0                                                      */
/*===========================================================================*/
ViStatus _VI_FUNC rsnrpz_GetSensorCount( ViSession iDummyHandle, ViInt32 *piNumConnectedSensors )
{
    ...
    if ( ! pAvailableSensor )
    {
        if ( piNumConnectedSensors )
            *piNumConnectedSensors = 0;
    ...
    if ( piNumConnectedSensors )
        *piNumConnectedSensors = iAvailableSensors;
...
/*                                                                           */
/*           iIndex is  1...n   where 'n' is the number of connected sensors.*/
/*                              'n' is evaluated by a call to                */
...
ViStatus _VI_FUNC rsnrpz_GetSensorInfo(
                      ...
                      ViInt32        iIndex,            // 1...n ( n = number of connected sensors )
                      ...
                      )
{
    ...
    if ( (1 > iIndex) || (iIndex > iAvailableSensors) )
        return VI_ERROR_PARAMETER2;

    if ( pszSensorName )
        strcpy( pszSensorName, pAvailableSensor[ iIndex-1 ].szLogicalName );

    if ( pszSensorType )
        strcpy( pszSensorType, pAvailableSensor[ iIndex-1 ].szSensorType );

    if ( pszSensorSerial )
        strcpy( pszSensorSerial, pAvailableSensor[ iIndex-1 ].szSerial );
  ...
  )
{
  ...
  // get associated session and check
  pmain_session = V2N_Lock( iHandle );
  if( pmain_session==0 )
    return VI_ERROR_PARAMETER1;
...
/*===========================================================================*/
/* Function: Set speed of USB interface of NRPxxS/A/T sensors                */
/* Purpose:  This function is used to reduce the HighSpeed (480 Mbit/s)      */
/*           transmission of the "new" sensors to FullSpeed (12 Mbit/s)      */
/*           which the NRP-Zxx series sensors were using                     */
/* Not supported for: VEE                                                    */
/*===========================================================================*/
ViStatus _VI_FUNC rsnrpz_setDeviceIFSpeed(
  ViSession   vi,
  ViInt32     channel,
  ViInt32     interfaceSpeed
)
{
  SMainSession *pmain_session = 0;
  ViStatus      rsnrpz_status = VI_SUCCESS;
  ...
  if ( (interfaceSpeed != RSNRPZ_IFSPEED_12M)
    && (interfaceSpeed != RSNRPZ_IFSPEED_480M)
     )
    return VI_ERROR_PARAMETER3;
 ...
 /*
  * Send IF Speed of a specified sensor
  */
  if ( pmain_session->pSessions[channel-1] )
    rsnrpz_status = NrpSetAttribute( pmain_session->pSessions[channel-1], NRP_ATTR_HIGHSPEED, (NRP_USERARG)interfaceSpeed );
...
// adding more type as the becomes released...
...
{
  /* cpVersion can contain 'old' or 'new' version scheme:
   *   Old:                1.37
   *    or                4.13b
   *    or          15.04.27.01
   *
   *   New:      02.00.18061401
   *    or  01.20.18061401.beta
   */
  const char *cpPart = 0;
  ...
  if ( *piSub < 100000 )
    return;   /* old version scheme */

  /* New version scheme
   * ---------------------------
   * SUB has at least 6 digits
   * Split the SUB into YY MM DD
   * and re-interprete again...
   */
  {
    char szSplit[50];
    memset( szSplit, 0x00, sizeof( szSplit ) );

    strncpy( szSplit, cpPart, 2 );
    strcat( szSplit, "." );
    strncpy( szSplit + 3 , cpPart + 2, 2 );
    strcat( szSplit, "." );
    strncpy( szSplit + 6 , cpPart + 4, 2 );

    SplitVersion( szSplit, piMajor, piMinor, piSub );
  }
}
                                           ...
                                           ViBoolean     *pbFirmwareOkay )
{
  ...
  ViChar            bufferType[30];
  ViChar            bufferSwBuild[50];
  int               iCurrMajor;
  int               iCurrMinor;
  int               iCurrSub;
  int               iRequiredMajor;
...
/* Test...
  SplitVersion( "1.20.180614", &iCurrMajor, &iCurrMinor, &iCurrSub );
  SplitVersion( "02.00.18092301.beta", &iCurrMajor, &iCurrMinor, &iCurrSub );
  SplitVersion( "15.04.10.01", &iCurrMajor, &iCurrMinor, &iCurrSub );
...
*/
  ...
  if ( rsnrpz_status != NRP_SUCCESS )
    return VI_WARN_UNKNOWN_SENSOR;

  if ( ! strlen( bufferType ) )
    return VI_WARN_UNKNOWN_SENSOR;  /* NRP_ERROR_INVALID_SENSOR_TYPE; */
  ...
  {
    strncpy( firmwareRequiredMinimum, "undefined", iBufSize );
    return VI_WARN_UNKNOWN_SENSOR;  // Unknown sensor type; not in our list
  }
...
/*===========================================================================*/
/* Function: Read Calibration Data                                           */
/* Purpose:  This function returns binary calibration data-block             */
/*                                                                           */
/*           The caller can't get ownership of the data(!)                   */
/*===========================================================================*/
#undef  __FUNC__
#define __FUNC__ "rsnrpz_calib_readData"
                                       ...
                                       )
{
  ...
  *pData = 0;   // No data so far

  if ( ! rsnrpz_IsSupported( vi, channel, 1, 1 ) )  //  1/1  =  CAL:DATA
    return RSNRPZ_ERROR_COMMAND_NOT_SUPPORTED;
  ...
  CHECKERR_CU( NrpSendCommand( pmain_session->pSessions[ channel-1 ],
                               "CAL:DATA?", 0 ) );

  dwError = WaitForSingleObject( pmain_session->hHandlesRd[0],
                                 dwTimeout );
  ...
  { /* inner block */
    ...
    if ( psession_info->ArrayBinaryDataSize > 0 )
    {
      *pData = psession_info->ArrayBinaryData;
      if ( pCount )
        *pCount = psession_info->ArrayBinaryDataSize;
    }
...
Error:
  ...
  {
    trace_log( "DestroyEvents 2 in " __FUNC__ );
    rsnrpz_DestroyEvents( vi, pmain_session->hHandlesRd, uiHandleCount );
  }
...
//-------------------------------------------------------
#undef  __FUNC__
#define __FUNC__ "rsnrpz_sensor_reboot"
ViStatus _VI_FUNC rsnrpz_sensor_reboot( ViSession vi,
                                        ViInt32   channel
                                      )
{
  ViStatus      rsnrpz_status = VI_SUCCESS;
  SMainSession *pmain_session = 0;
  ViInt32       VrtResetAll   = 1;
  ...
  if ( rsnrpz_IsSupported( vi, channel, 20, 4 ) )  //  20/4  =  SYSTEM:REBOOT
  {
    rsnrpz_status = rsnrpz_WriteCommand( vi, channel, (char*)"SYST:REB", NULL );
  }
  else
  {
    // This leads to a reboot of the sensor...
    NrpSendVendorOutRequest( pmain_session->pSessions[channel-1],
                             VrtResetAll,
                             1,
                             0
                           );

    // ...therefore we close that session (without any further communication)
    rsnrpz_status = rsnrpz_close_session( vi, VI_FALSE );
  }
...
/*===========================================================================*/
/* Function: Sets a User Selected Sensor Name                                */
/*           This function is for the "new" sensor generation only           */
/*           (NRPxxS[N], NRPxxA[N], NRPxxT[N])                               */
/*                                                                           */
/* Purpose:  Sets a name of the sensors according to the users requirements. */
/*           The name specified by 'pName' will be displayed on the WebGUI   */
/*           of the network sensors.                                         */
/*           The sensor name given here is independent from the hostname     */
/*           of the sensor. However, if the sensor name is not yet specified */
/*           it defaults to the hostname                                     */
/*===========================================================================*/
#undef  __FUNC__
#define __FUNC__ "rsnrpz_setSensorName"
ViStatus _VI_FUNC rsnrpz_setSensorName( ViSession instrHdl, ViInt32 chan, const ViChar *pName )
{
  ViStatus      rsnrpz_status = VI_SUCCESS;
  SMainSession *pmain_session = 0;
  ViChar       *pDynBuf       = 0;
  ...
  pmain_session = V2N_Lock( instrHdl );
  if( pmain_session == 0 )
    return VI_ERROR_PARAMETER1;
  ...
  if ( ! pmain_session->pSessions[ chan-1 ] )
    return VI_ERROR_PARAMETER2;

  if ( ! pName )
    return VI_ERROR_PARAMETER3;

  if ( ! rsnrpz_IsSupported( instrHdl, chan, 20, 27 ) )  //  20/27  =  SYST[:SENS]:NAME
    return RSNRPZ_ERROR_COMMAND_NOT_SUPPORTED;


  rsnrpz_GetIndexFromPacket( 20, 27, &index);
  if ( index >= 0 )
    pmain_session->pSInfos[ chan-1 ]->CacheTable[index].type = DATA_STRING;

  pDynBuf = (ViChar*)malloc( strlen( pName ) + 3 );
  memset( pDynBuf, 0, strlen( pName ) + 3 );
  strcpy( pDynBuf, "\"" );
  strcat( pDynBuf, pName );
  strcat( pDynBuf, "\"" );
  rsnrpz_status = rsnrpz_WriteCommand( instrHdl, chan, (char*)"SYST:NAME", pDynBuf );
  free( pDynBuf );

  if ( rsnrpz_status != VI_SUCCESS )
    return rsnrpz_status;

  rsnrpz_status = rsnrpz_checkStatus( instrHdl );
...
//-------------------------------------------------------
#undef  __FUNC__
#define __FUNC__ "rsnrpz_getSensorName"
ViStatus _VI_FUNC rsnrpz_getSensorName( ViSession instrHdl, ViInt32 chan, ViChar *pName, ViUInt32 maxLen )
{
  ViStatus      rsnrpz_status = VI_SUCCESS;
  SMainSession *pmain_session = 0;
  ViInt32       iCount        = 0;
  ...
  iCount = rsnrpz_ReadValue( instrHdl, chan, (char*)"SYST:NAME", szDummy, 0 );
  if ( iCount > 0 )
  {
    ViChar *pDynBuf = (ViChar*)malloc( iCount );
    memset( pDynBuf, 0, iCount );
    rsnrpz_status = rsnrpz_ReadValue( instrHdl, chan, (char*)"SYST:NAME", pDynBuf, iCount );
    if ( pName )
    {
      strncpy( pName, pDynBuf, maxLen );
      pName[ maxLen - 1 ] = 0x00;
    }
...
//-------------------------------------------------------
#undef  __FUNC__
#define __FUNC__ "rsnrpz_setLedMode"
ViStatus _VI_FUNC rsnrpz_setLedMode( ViSession instrHdl, ViInt32 chan, ViUInt32 ledMode )
{
  ViStatus      rsnrpz_status = VI_SUCCESS;
  SMainSession *pmain_session = 0;
  ...
  if ( ! rsnrpz_IsSupported( instrHdl, chan, 20, 26 ) )  //  20/26  =  SYST:LED:MODE
    return RSNRPZ_ERROR_COMMAND_NOT_SUPPORTED;

  rsnrpz_status = rsnrpz_WriteCommand( instrHdl, chan, (char*)"SYST:LED:MODE", &ledMode );
  if ( rsnrpz_status != VI_SUCCESS )
    return rsnrpz_status;
...
//-------------------------------------------------------
#undef  __FUNC__
#define __FUNC__ "rsnrpz_getLedMode"
ViStatus _VI_FUNC rsnrpz_getLedMode( ViSession instrHdl, ViInt32 chan, ViUInt32 *pLedMode )
{
  ViStatus      rsnrpz_status = VI_SUCCESS;
  SMainSession *pmain_session = 0;
  ViUInt32      value         = 0;
  ...
  CHECKERR( rsnrpz_ReadValue( instrHdl, chan, (char*)"SYST:LED:MODE", &value, sizeof(value) ) );

  if ( pLedMode )
    *pLedMode = value;

  CHECKERR( rsnrpz_checkStatus( instrHdl ) );
...
//-------------------------------------------------------
#undef  __FUNC__
#define __FUNC__ "rsnrpz_setLedColor"
ViStatus _VI_FUNC rsnrpz_setLedColor( ViSession instrHdl, ViInt32 chan, ViUInt32 color )
{
  ViStatus      rsnrpz_status = VI_SUCCESS;
  SMainSession *pmain_session = 0;
  ...
  if ( ! rsnrpz_IsSupported( instrHdl, chan, 20, 25 ) )  //  20/25  =  SYST:LED:COL
    return RSNRPZ_ERROR_COMMAND_NOT_SUPPORTED;

  color = color & 0x00ffffff; // no alpha channel
  rsnrpz_status = rsnrpz_WriteCommand( instrHdl, chan, (char*)"SYST:LED:COL", &color );
  if ( rsnrpz_status != VI_SUCCESS )
    return rsnrpz_status;
...
//-------------------------------------------------------
#undef  __FUNC__
#define __FUNC__ "rsnrpz_getLedColor"
ViStatus _VI_FUNC rsnrpz_getLedColor( ViSession instrHdl, ViInt32 chan, ViUInt32 *pColor )
{
  ViStatus      rsnrpz_status = VI_SUCCESS;
  SMainSession *pmain_session = 0;
  ViUInt32      value         = 0;
  ...
  CHECKERR( rsnrpz_ReadValue( instrHdl, chan, (char*)"SYST:LED:COL", &value, sizeof(value) ) );

  if ( pColor )
    *pColor = (value & 0x00ffffff);
...
//-------------------------------------------------------
#undef  __FUNC__
#define __FUNC__ "rsnrpz_trigger_setMasterPort"
ViStatus _VI_FUNC rsnrpz_trigger_setMasterPort( ViSession instrHdl, ViInt32 chan, ViUInt32 port )
{
  ViStatus      rsnrpz_status = VI_SUCCESS;
  SMainSession *pmain_session = 0;
  ...
  if ( ! rsnrpz_IsSupported( instrHdl, chan, 20, 28 ) )  //  20/28  =  TRIG:MAST:PORT
    return RSNRPZ_ERROR_COMMAND_NOT_SUPPORTED;

  rsnrpz_status = rsnrpz_WriteCommand( instrHdl, chan, (char*)"TRIG:MAST:PORT", &port );
  if ( rsnrpz_status != VI_SUCCESS )
    return rsnrpz_status;
...
//-------------------------------------------------------
#undef  __FUNC__
#define __FUNC__ "rsnrpz_trigger_getMasterPort"
ViStatus _VI_FUNC rsnrpz_trigger_getMasterPort( ViSession instrHdl, ViInt32 chan, ViUInt32 *port )
{
  ViStatus      rsnrpz_status = VI_SUCCESS;
  SMainSession *pmain_session = 0;
  ViUInt32      value         = 0;
  ...
  CHECKERR( rsnrpz_ReadValue( instrHdl, chan, (char*)"TRIG:MAST:PORT", &value, sizeof(value) ) );
...
//-------------------------------------------------------
#undef  __FUNC__
#define __FUNC__ "rsnrpz_trigger_setSyncPort"
ViStatus _VI_FUNC rsnrpz_trigger_setSyncPort( ViSession instrHdl, ViInt32 chan, ViUInt32 port )
{
  ViStatus      rsnrpz_status = VI_SUCCESS;
  SMainSession *pmain_session = 0;
  ...
  if ( ! rsnrpz_IsSupported( instrHdl, chan, 20, 29 ) )  //  20/29  =  TRIG:SYNC:PORT
    return RSNRPZ_ERROR_COMMAND_NOT_SUPPORTED;

  rsnrpz_status = rsnrpz_WriteCommand( instrHdl, chan, (char*)"TRIG:SYNC:PORT", &port );
  if ( rsnrpz_status != VI_SUCCESS )
    return rsnrpz_status;
...
//-------------------------------------------------------
#undef  __FUNC__
#define __FUNC__ "rsnrpz_trigger_getSyncPort"
ViStatus _VI_FUNC rsnrpz_trigger_getSyncPort( ViSession instrHdl, ViInt32 chan, ViUInt32 *port )
{
  ViStatus      rsnrpz_status = VI_SUCCESS;
  SMainSession *pmain_session = 0;
  ViUInt32      value         = 0;
  ...
  CHECKERR( rsnrpz_ReadValue( instrHdl, chan, (char*)"TRIG:SYNC:PORT", &value, sizeof(value) ) );
...
//-------------------------------------------------------
/*===========================================================================*/
/* Function: rsnrpz_getUsageMap                                              */
/*                                                                           */
/* Purpose:  Starting with revision 3.2.1.0 of this driver, the underlying   */
/*           low level library has been made usable by multiple applications */
/*           concurrently. A sensor which is already in use by another       */
/*           running application cannot by initialized by a different        */
/*           In this case the rsnrpz_init() function would return a status   */
/*           NRP_ERROR_SENSOR_IN_USE (0xC0000022).                           */
/*                                                                           */
/*           rsnrpz_getUsageMap() can be used to get a textual description   */
/*           of the current sensor & application relationship.               */
/*           For a sensor which is already in use, the application which     */
/*           holds the sensor, is shown in the usage map. This information   */
/*           can, for example, be presented to a user.                       */
/*                                                                           */
/*           HINT: Due to syntax definitions the first parameter must be of  */
/*                 type 'ViSession', but it is not used and should be chosen */
/*                 as 0                                                      */
/*                                                                           */
/*           The textual usage map is returned via a char buffer 'cpMap'     */
/*           which should be supplied by the caller. 'maxLen' should be set  */
/*           to the size of the user's buffer in order to limit the amount   */
/*           of data which will be copied to 'cpMap'.
/*           If 'pRetLen' is not NULL, the current length of the usage map   */
/*           is returned here (which may be longer than 'maxLen'.            */
/*===========================================================================*/
#undef  __FUNC__
#define __FUNC__ "rsnrpz_getUsageMap"
ViStatus _VI_FUNC rsnrpz_getUsageMap( ViSession iDummyHandle, ViChar cpMap[], ViUInt32 maxLen, ViUInt32 *pRetLen )
{
  long          lNrpStatus = NRP_SUCCESS;
  ...
  lNrpStatus = NrpGetUsageMap( cpMap, maxLen, &ulRet );
  if ( pRetLen )
    *pRetLen = ulRet;
...
/*===========================================================================*/
/* Function: Write Calibration Data                                          */
/* Purpose:  This function writes binary calibration data and resets the     */
/*           device (in order to have the device using the new data)         */
/*                                                                           */
/*           The device goes away (disconnects) after the data has been      */
/*           written. As a consequence, the session will be closed and       */
/*           thus becomes invalid(!)                                         */
/*===========================================================================*/
#undef  __FUNC__
#define __FUNC__ "rsnrpz_calib_writeData"
                                        ...
                                        )
{
  ...
  memset( pmain_session->hHandlesWr, 0, sizeof( pmain_session->hHandlesWr ) );

  rsnrpz_status = rsnrpz_GetIndexFromCommand( (char*)"CAL:DATA", &iCmdIdx );
  ...
  CHECKERR( rsnrpz_CreateEvents( vi,
                                 channel,
                                 gs_NrpCommands[iCmdIdx].groupNb,
                                 gs_NrpCommands[iCmdIdx].paramNb,
                                 pmain_session->hHandlesWr,
                                 &uiHandleCount ) );
  ...
  CHECKERR_CU( NrpSendBinaryBlock( pmain_session->pSessions[channel-1],
                                   "CAL:DATA",
                                   (void*)pData,
                                   arraySize,
                                   dwTimeout ) );

  dwError = WaitForSingleObject( pmain_session->hHandlesWr[0], dwTimeout );
...
Error:
  ...
  {
    trace_log( "DestroyEvents 3 in " __FUNC__ );
    rsnrpz_DestroyEvents( vi, pmain_session->hHandlesWr, uiHandleCount );
  }

  if ( dwError == WAIT_TIMEOUT )
    return RSNRPZ_ERROR_TIMEOUT;

  if ( rsnrpz_status == VI_SUCCESS )
    return rsnrpz_sensor_reboot( vi, channel );
...
/*===========================================================================*/
/* Function: Read User Calibration Data                                      */
/* Purpose:  This function returns binary calibration data-block             */
/*                                                                           */
/*           The caller can't get ownership of the data(!)                   */
/*===========================================================================*/
#undef  __FUNC__
#define __FUNC__ "rsnrpz_calib_readUserData"
ViStatus _VI_FUNC rsnrpz_calib_readUserData( ViSession      vi,
											                       ViInt32        channel,
		                                         const ViChar **pData,
                                             ViInt32       *pCount
                                           )
{
  ...
  *pData = 0;   // No data so far
  ...
  if ( ! rsnrpz_IsSupported( vi, channel, 20, 1 ) )  //  20/1  =  CAL:USER:DATA
    return RSNRPZ_ERROR_COMMAND_NOT_SUPPORTED;
...
/*===========================================================================*/
/* Function: Write User Calibration Data                                     */
/* Purpose:  This function writes binary calibration data and resets the     */
/*           device (in order to have the device using the new data)         */
/*                                                                           */
/*           The device goes away (disconnects) after the data has been      */
...
/*===========================================================================*/
#undef  __FUNC__
#define __FUNC__ "rsnrpz_calib_writeUserData"
ViStatus _VI_FUNC rsnrpz_calib_writeUserData( ViSession     vi,
                                              ViInt32       channel,
                                              const ViChar *pData,
                                              ViInt32       arraySize
                                            )
{
  ...
  rsnrpz_status = rsnrpz_GetIndexFromCommand( (char*)"CAL:USER:DATA", &iCmdIdx );
